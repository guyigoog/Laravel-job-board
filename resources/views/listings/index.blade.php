    ...
    <section class="container px-5 py-12 mx-auto">
        <div class="mb-12">
            <div class="flex-justify-center">
                @foreach ($tags as $tag)
                    <a href="{{ route('listings.index', ['tag' => $tag->slug]) }}" class="inline-block px-3 py-1 mr-2 text-xs font-medium leading-6 text-center text-indigo-700 uppercase {{$tag -> slug == request()->get('tag') ? 'bg-indigo-400 text-white ' : 'bg-indigo-100 text-indigo-500 ' }} transition rounded-full shadow ripple hover:shadow-lg hover:bg-indigo-200 focus:outline-none">
                        {{ $tag->name }}
                    </a>
                @endforeach
            </div>
